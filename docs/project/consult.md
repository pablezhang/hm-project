...
## 极速问诊-静态结构搭建{#consult-change-type}
...
## 极速问诊-选择科室-结构和样式{#consult-dep-html}
...
<template>
  ...
  <div class="illness-form">
    ...
    <div class="item">
      <p>本次患病多久了？</p>
      <cp-radio :options="timeOptions" />
    </div>
    <div class="item">
      <p>此次病情是否去医院就诊过？</p>
      <cp-radio :options="flagOptions" />
    </div>
  </div>
</template>
...
const disabled = computed(
   // illnessDesc illnessTime illnessTime均为必选
  () =>
...
const next = () => {
  ...
  // 跳转档案管理（复用），需要根据 needSelect 实现选择功能
  router.push('/user/patient?needSelect=1')
}
```
...
## 选择患者-家庭档案（复用）{#consult-change-patient}
...
const isNeedSelect = computed(() => route.query.isNeedSelect === '1')
```
```html
<cp-nav-bar :title="isNeedSelect ? '选择患者' : '家庭档案'" />
```
```html
    <!-- 头部提示 -->
    <div class="patient-change" v-if="isNeedSelect">
      <h3>请选择患者信息</h3>
...
```html
    <!-- 底部按钮 -->
    <div class="patient-next" v-if="isNeedSelect">
