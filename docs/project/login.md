...
## 组件封装-cp-nav-bar {#cp-nav-bar-html}
...
- sass中[&的作用](https://www.sass.hk/docs/)

- ```sass
  .foo {
    
    &_xxx {  	}	 // &指代父级类名， 是sass和less提供的简写方式
  }
...
## 组件封装-cp-nav-bar 组件功能{#cp-nav-bar-logic}
...
+ //  1. 定义props类型
+ interface ICPNavBarProps {
+   title?: string
+   rightText?: string
+ }

+ // 2. 定义props
+ defineProps<ICPNavBarProps>()
...
## 组件封装-cp-nav-bar 组件类型{#cp-nav-bar-type}
...
const useUserStore = defineStore('cp-user', () => {
  ...
  const saveUserInfo = (u: IUserInfo) => {
    userInfo.value = u
  }
...
<div class="login-head">
-  <h3>密码登录</h3>
+  <h3>{{ store.isPass ? '密码登录' : '短信验证码登录' }}</h3>
